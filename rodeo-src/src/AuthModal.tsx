...
export function AuthModal({ onClose }: AuthModalProps) {
  ...
  const handleForgotPassword = async () => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/rodeo/`,
    })
    setLoading(false)
    if (error) { setError(error.message); return }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "24px 24px 0", display: "flex", alignItems: "center", gap: 12 }}>
          <img src="/rodeo/rodeologo.png" alt="Rodeo AI" height={36} style={{ display: "block", objectFit: "contain" }} />
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: "0.1em", color: "#18181b" }}>RODEO AI</div>
            <div style={{ fontSize: 12, color: "#71717a" }}>Multi-model conversation</div>
          </div>
        </div>
        ...
        <form onSubmit={tab === "login" ? handleLogin : handleSignup} style={{ padding: "16px 24px 24px", display: "grid", gap: 12 }}>
          ...
          <button type="submit" disabled={loading}
            style={{ padding: "11px", border: "1px solid #3296ab", borderRadius: 4, background: "#3296ab", color: "#fff", fontFamily: "inherit", fontWeight: 600, fontSize: 14, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}>
