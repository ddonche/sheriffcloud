...
serve(async (req) => {
  ...
  if (!content && !attachmentUrl) return json(400, { ok: false, error: "content or attachment_url is required" })
  ...
  // ── Billing pre-flight ────────────────────────────────────────────────────
  // Check if every participant in this session has a BYOK key.
  // If all do, skip billing checks entirely.
  const participants: string[] = Array.isArray(session.participants) ? session.participants : []

  const providerMap: Record<string, string> = {
    chatgpt: "openai", gemini: "gemini", claude: "claude", grok: "grok",
  }
  const providers = [...new Set(participants.map(p => providerMap[p] ?? p))]
  ...
  const savedProviders = new Set((savedKeys ?? []).map((r: { provider: string }) => r.provider))
  const allByok = providers.every(p => savedProviders.has(p))

  if (!allByok) {
    // At least one provider will use master keys — check free tier / credits
  ...
  if (pauseSession) {
    ...
    if (pauseError || !paused) {
      return json(500, { ok: false, error: pauseError?.message ?? "Failed to pause session", message })
    }

    updatedSession = paused
  }

  return json(200, { ok: true, session: updatedSession, message })
})
